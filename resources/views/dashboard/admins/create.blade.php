...
<form action="{{ route('admin.admins.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row">
        ...
        <div class="col-md-6 col-sm-12">
            ...
            <input class="form-control  @error('country_code') is-invalid error('phone_code') is-invalid @enderror @error('phone') is-invalid @enderror" type="tel" name="phone" id="phone_number" placeholder="{{ __('Phone Number') }}" value="{{ old('phone') }}" required autocomplete="phone">
        ...
        <div class="form-group col-md-6">
            ...
            <label for="file" class="file-input btn btn-block btn-primary btn-file w-100">
                @lang('Browse')
                <input accept="image/*" type="file" name="image" id="file" onchange="document.getElementById('image').src = window.URL.createObjectURL(this.files[0])">
            </label>
        </div>
        <div class="col-md-6 col-sm-12">
            <label for="client_password">{{ __('password') }}</label>
            <input required type="password" name="password" parsley-trigger="change" class="form-control" data-parsley-id="10">
        </div>
        <div class="col-md-6 col-sm-12">
            <label for="client_password">{{ __('confirmPassword') }}</label>
