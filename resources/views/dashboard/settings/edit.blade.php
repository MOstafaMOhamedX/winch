...
<form method="POST" action="{{ route('admin.type.settings.update',['setting' => 1, 'type' => request()->type]) }}" enctype="multipart/form-data" data-parsley-validate novalidate>
    @csrf
    @method('PUT')
    <div class="row">
        @foreach ($Settings as $Setting)
            @if (str_contains($Setting['key'], 'image') || $Setting['type'] == 'image' || in_array($Setting['key'], ['logo','watermark']))
                <div class="col-sm-12">
                    <div class="form-group">
                        <label for="{{ $Setting['key'] }}">@lang($Setting['key'])</label>
            ...
            @elseif(in_array($Setting['key'], ['login','register','show_sizes','branches_status','order_notifications']) || str_contains($Setting['key'],'display'))
                <div class="col-sm-12">
                    <div class="form-group col-md-12">
                        ...
                        <label for="{{ $Setting['key'] }}">@lang('status')</label>
                        @else
                        <label for="{{ $Setting['key'] }}">@lang('' . $Setting['key'])</label>
                        @endif
                        <select id="{{ $Setting['key'] }}" name="{{ $Setting['key'] }}" required class="form-control">
                            <option {{ $Setting['value'] == 0 ? 'selected' : '' }} value="0">@lang('hidden')</option>
                            <option {{ $Setting['value'] == 1 ? 'selected' : '' }} value="1">@lang('visible')</option>
                        </select>
                    </div>
                </div>
            @elseif(in_array($Setting['key'], ['accept_order']))
                <div class="col-sm-12">
                    <div class="form-group col-md-12">
                        <label for="{{ $Setting['key'] }}">@lang('' . $Setting['key'])</label>
                        <select id="{{ $Setting['key'] }}" name="{{ $Setting['key'] }}" required class="form-control">
                            <option {{ $Setting['value'] == 0 ? 'selected' : '' }} value="0">@lang('no')</option>
                            <option {{ $Setting['value'] == 1 ? 'selected' : '' }} value="1">@lang('yes')</option>
                        </select>
                    </div>
                </div>
            @elseif ((str_contains($Setting['key'], 'ar') || str_contains($Setting['key'], 'en') || $Setting['type'] == 'meta') && !str_contains($Setting['key'], 'title_'))
                <div class="col-sm-12">
                    <div class="form-group">
                        <label for="{{ $Setting['key'] }}">@lang($Setting['key'])</label>
                        <textarea style="min-height: 300px" class="form-control mceNoEditor" id="{{ $Setting['key'] }}" name="{{ $Setting['key'] }}" required placeholder="@lang('' . $Setting['key'])">{{ strip_tags($Setting['value']) }}</textarea>
                    </div>
                </div>
            ...
            @elseif(str_contains($Setting['key'], 'color'))
                <div class="col-sm-12">
                    <div class="form-group">
                        <label for="{{ $Setting['key'] }}">@lang($Setting['key'])</label>
                        <input id="{{ $Setting['key'] }}" type="color" name="{{ $Setting['key'] }}" required placeholder="@lang('' . $Setting['key'])" class="form-control" value="{{ $Setting['value'] }}">
                    </div>
                </div>
            @else
                <div class="col-sm-12">
                    <div class="form-group">
                        <label for="{{ $Setting['key'] }}">@lang($Setting['key'])</label>
                        <input id="{{ $Setting['key'] }}" type="text" name="{{ $Setting['key'] }}" required placeholder="@lang('' . $Setting['key'])" class="form-control" value="{{ $Setting['value'] }}">
                    </div>
                </div>
            @endif
        @endforeach
